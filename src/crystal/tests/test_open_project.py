...
async def test_given_project_was_just_opened_then_no_resources_loaded_except_root_resources() -> None:
    def loaded_resources(project: Project) -> List[Resource]:
    ...
    with extracted_project('testdata_xkcd.crystalproj.zip') as project_dirpath:
        async with (await OpenOrCreateDialog.wait_for()).open(project_dirpath) as mw:
            ...
            resources_that_are_root = sorted(
            ...
            if True:
                ...
                resources_that_are_root_or_group_members = sorted(
...
async def test_when_resource_group_node_expanded_then_only_new_resources_loaded_are_group_members() -> None:
    pass
