...
_FAKE_BOTO3_PATH = os.path.join(
    os.path.dirname(__file__), os.pardir, 'util', 'fake_boto3'
)

_BOTO_MODULE_KEYS = ['boto3', 'botocore', 'botocore.exceptions']
...
def _fake_s3(
    ...
    """
    fake_boto3_abspath = os.path.abspath(_FAKE_BOTO3_PATH)
    ...
    saved_modules = {k: sys.modules.pop(k) for k in _BOTO_MODULE_KEYS if k in sys.modules}

    os.environ.update(env_add)
    for k in env_remove:
        os.environ.pop(k, None)
    sys.path.insert(0, fake_boto3_abspath)
    try:
        yield
    finally:
        sys.path.remove(fake_boto3_abspath)
        for k in _BOTO_MODULE_KEYS:
...
def _crystal_running_with_fake_s3(
    ...
    * omit_env_var_credentials --
    ...
    base_env = {
        'PYTHONPATH': _prepend_to_pythonpath(os.path.abspath(_FAKE_BOTO3_PATH)),
...
def _prepend_to_pythonpath(new_entry: str) -> str:
    existing = os.environ.get('PYTHONPATH', '')
    if existing:
        return f'{new_entry}:{existing}'
    else:
        return new_entry
